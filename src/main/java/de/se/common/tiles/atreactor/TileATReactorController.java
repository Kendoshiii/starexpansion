...
public class TileATReactorController extends TileEntity implements ITickable{
	...
	private List<Block> acceleratorBlocks = new ArrayList<Block>();
	...
	public TileATReactorController()
	{
		structureBlocks.add(SEBlocks.STEEL_CASE);
		structureBlocks.add(SEBlocks.ATREACTOR_CONTROLLER);
		structureBlocks.add(SEBlocks.ATREACTOR_POWER_CONNECTOR);
		structureBlocks.add(SEBlocks.RADIANT_GLASS);
		
		casingBlocks.add(SEBlocks.STEEL_CASE);
		casingBlocks.add(SEBlocks.ATREACTOR_CONTROLLER);
		casingBlocks.add(SEBlocks.ATREACTOR_POWER_CONNECTOR);
		...
		for (int x = start.getX(); x <= end.getX(); ++x)
		{
			for (int z = start.getZ(); z <= end.getZ(); ++z)
			{
				for (int y = start.getY(); y >= end.getY(); --y)
				{
					...
					if (b == SEBlocks.ATREACTOR_POWER_CONNECTOR)
					{
						TileEntity te = this.world.getTileEntity(curPos);
						if (te instanceof TileATReactorPowerConnector)
						{
							((TileATReactorPowerConnector)te).setController(this);
						}
					}
					
					if (b == Blocks.GLASS || b == Blocks.STAINED_GLASS)
					{
						this.world.setBlockState(curPos, SEBlocks.RADIANT_GLASS.getDefaultState());
					}
				}
			}
		}
		...
		for (int x = xStart; x <= xEnd; ++x)
		{
			for (int z = zStart; z <= zEnd; ++z)
			{
				for (int y = yStart; y >= yEnd; --y)
				{
					...
					if (b == SEBlocks.ATREACTOR_CONTROLLER)
						hasController = true;
					...
					{
						if (!getGeneratorBlocks().contains(b))
						{
							isReactor = false;
							break box_check;
						}
						if (b == SEBlocks.RADIANT_BLOCK){
							++generatorBlockCount;
						}
					}
				}
	 ...
	 * Getters and Setters 
	 */
	public List<Block> getGeneratorBlocks() {
		return generatorBlocks;
	}

	public void setGeneratorBlocks(List<Block> generatorBlocks) {
